...
public class SupermarketJourneyPlanner implements IJourneyPlanner {
    ...
    public SupermarketJourneyPlanner() {
        initModeUtilities();
        initModeChangeCosts();
    ...
    public Journey generateJourneys(TrafficParticipantBDI agent) {
        ...
        if(!agent.isFromPopuplation()){
            ...
            //get Food Pricing Preference
            Map<Integer, Double> priceScores = this.parseScoresFromInd(ontoUtilBasics, infOnt, myself, "hasPriceScore");
            Map<Integer, Double> priceProbabilities = this.determineProbabilities(priceScores);
            int pricePreferenceValue = this.determinePreferenceValue(priceProbabilities);
            Preference pricePref = new Preference("FoodPrice", myself, priceScores, priceProbabilities, pricePreferenceValue);

            //get Food Quality Preference
            Map<Integer, Double> qualityScores = this.parseScoresFromInd(ontoUtilBasics, infOnt, myself, "hasQualityScore");
            Map<Integer, Double> qualityProbabilities = this.determineProbabilities(qualityScores);
            int qualityPreferenceValue = this.determinePreferenceValue(qualityProbabilities);
            Preference qualityPref = new Preference("FoodQuality", myself, qualityScores, qualityProbabilities, qualityPreferenceValue);
            ...
            decisionFactors.put(pricePref.getName(), pricePref.getPreferenceValue());
            decisionFactors.put(qualityPref.getName(), qualityPref.getPreferenceValue());
    ...
    public GroceryShoppingJourney visitSupermarket(GroceryShoppingJourney currentJourney, Location currentLocation, Supermarket targetSupermarket, Vehicle currentVehicle) {
        ...
        //determine shopping Utility
        int foodQualityPref = getPrefVal("FoodQuality");
        int foodPricePref = getPrefVal("FoodPrice");
        double shoppingUtility = foodPricePref * targetSupermarket.getPriceTendencyScore() + foodQualityPref * targetSupermarket.getProductQualityScore();
    ...
    private List<Supermarket> createSupermarketObjects(List<FinalDestination> possibleDestinations, OWLOntology inferredOntology) {
        ...
        for (FinalDestination poi : supermarketLocations) { //for each poi create SupermarketObject
            ...
            supermarkets.add(new Supermarket(poi, inventory, size, productQuality, priceTendency));
        }
