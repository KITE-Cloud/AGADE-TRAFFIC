...
public class SupermarketJourneyPlannerCoronaRestrictions implements IJourneyPlanner {
    ...
    final double maxMovingRadius = 2000;

    public SupermarketJourneyPlannerCoronaRestrictions() {
        initModeUtilities();
        initModeChangeCosts();
    ...
    public GroceryShoppingJourney selectNextSupermarket(GroceryShoppingJourney jo, List<Supermarket> remainingSupermarkets, Location currentLocation) {
        ...
        //determine supermarkets within moving radius
        Supermarket nearestSupermarket = new Supermarket();
        double nearestSupermarketDistance = Double.MAX_VALUE;
        List<Supermarket> possibleSuperMarkets = new ArrayList<>();
        List<GroceryShoppingJourney> possibleJourneysInRadius = new ArrayList<>();
        for (Supermarket supermarket : supermarkets) {
            double estimatedDist = MathUtil.distFrom(supermarket.getLocation().getLocation().getLat(), supermarket.getLocation().getLocation().getLon(), agent.getTrips().get(0).getStart().getLat(), agent.getTrips().get(0).getStart().getLon());
            if (estimatedDist < nearestSupermarketDistance) {
                nearestSupermarketDistance = estimatedDist;
                nearestSupermarket = supermarket;
            }
            if (estimatedDist <= maxMovingRadius) {
                possibleSuperMarkets.add(supermarket);
            }
        }
        if (possibleSuperMarkets.isEmpty()) {
            possibleSuperMarkets.add(nearestSupermarket);
        }

        //remove all journeys with supermarkets out of reach
        for (GroceryShoppingJourney journey : possibleJourneys) {
            boolean inRadius = true;
            for (Supermarket supermarket : journey.getVisitedSupermarket()) {
                if (!possibleSuperMarkets.contains(supermarket)) {
                    inRadius = false;
                    break;
                }
            }
            if (inRadius) {
                possibleJourneysInRadius.add(journey);
            }
        }


        //choose journey with totalUtilityScore
        possibleJourneysInRadius.sort(GroceryShoppingJourney::compareTo);

        if (possibleJourneysInRadius.size() == 0) {
            return jo; //last journey selected
        } else {
            return possibleJourneysInRadius.get(possibleJourneysInRadius.size() - 1);
        }
    }
    ...
    public double determineTotalUtilityScoreOfJourney(GroceryShoppingJourney journey, MinMaxOfJourneysObject minMax, double productCoverage) {
        ...
        double totalScore = (1 - totalTravelDistanceScaled) + avgModeUtilityScaled + avgShoppingUtilityScaled + (coverageImportance * productCoverage);
        return totalScore;
    }
